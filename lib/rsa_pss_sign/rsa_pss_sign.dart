...
class HashName {
  ...
  const HashName(this.name);

  static const sha256 = HashName('sha256');
}
...
Future<List<int>> signRsaPss(String privateKey, String message,
    {HashName digest = HashName.sha256, int saltLength = 20}) async {
  ...
  if (res.exitCode != 0) {
    ...
    throw OpensslException(res.exitCode, systemEncoding.decode(res.stdout),
        res.stderr);
  }
